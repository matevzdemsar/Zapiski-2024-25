...
#
# **Opomba:** Tomo ne more preveriti pravilnosti vaših rešitev, zato
# bo rešitve vedno sprejel kot pravilne.
# =====================================================================@005979=
# 1. podnaloga
# Zgornji program `optimizator.py` nadgradite s simuliranim ohlajanjem:
# 
...
class Delci():
    ...
    def potencial(self, x):
        ...
        zlobniFaktor = 0.2 # Večji faktor povzorči večje hribčke v energijski funkciji
...
class DelciOhlajanje():
    ...
    def __init__(self, n, info, canvas):
        ...
        self.vozlisce = [(random.random(), random.random()) for i in range(n)]
        self.krogec = []
        self.zacetna_temp = 20000
        self.faktor_ohlajanja = 0.90
        for p in self.vozlisce:
            x = WIDTH * p[0]
            y = HEIGHT * p[1]
            self.krogec.append(canvas.create_oval(x-3, y-3, x+3, y+3))
    ...
    def potencial(self, x):
        """Potencialna energija delca glede na drugi delec, ki je od njega oddaljen x."""
        zlobniFaktor = 20 # Večji faktor povzorči večje hribčke v energijski funkciji
        return 1/(50*x) + zlobniFaktor * math.cos(8 * math.pi * x) + x
    ...
    def temperatura(self):
        return self.zacetna_temp * self.faktor_ohlajanja ** self.korak

    def optimiraj(self):
        for (k,p) in enumerate(self.vozlisce):
            q = self.perturbacija(p)
            if self.spremembaEnergije(k,q) < 0.0 or random.random() < 1 / (1 + math.exp(- (self.energija() / self.temperatura) ** 2)):
                self.vozlisce[k] = q
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1OTc5LCJ1c2VyIjo5MTQyfQ:1uKFjB:bKRA1H3xxjQVfnuXeBXCdMOM05YoZpr5RvMUBOP-98w"
        try:
