...
#     False
# =============================================================================
je_prestopno = lambda leto: True if not (leto % 400) else False if not (leto % 100) else True if not (leto % 4) else False
# =====================================================================@005837=
# 2. podnaloga
...
# =============================================================================
stevilo_dni = lambda leto: 365 + je_prestopno(leto)
# =====================================================================@005838=
# 3. podnaloga
...
# =============================================================================
dolzine_mesecev = lambda leto: [31, 29 if je_prestopno(leto) else 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
# =====================================================================@005839=
# 4. podnaloga
...
# =============================================================================
    def je_veljaven(self):
        if self.mesec - 1 not in range(12):
            return False
        elif self.dan - 1 not in range(dolzine_mesecev(self.leto)[self.mesec - 1]):
            return False
        else: return True
# =====================================================================@005843=
# 8. podnaloga
...
# =============================================================================
    def dan_v_letu(self):
        if self.je_veljaven():
            return sum(dolzine_mesecev(self.leto)[:self.mesec - 1]) + self.dan
        else: return "Treason! To the guillotine!"
# =====================================================================@005846=
# 11. podnaloga
...
# =============================================================================
    ...
    def dni_od_zacetka(self):
        return 365 * (self.leto - 1) + (self.leto - 1) // 4 - (self.leto - 1) // 100 + (self.leto - 1) // 400 + self.dan_v_letu()
...
# =============================================================================
    def dan_v_tednu(self):
        dan = (self.razlika(Datum(16, 4, 2025)) + 3) % 7
        return dan if dan else 7
# =====================================================================@005849=
# 13. podnaloga
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1ODM2LCJ1c2VyIjo5MTQyfQ:1u51oI:dOB4tWH2HOXUDzAK_MDowhls5kWOUKQf8g47vio-eNY"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1ODM3LCJ1c2VyIjo5MTQyfQ:1u51oI:zksnjgMOxMOFi5tRLnBRGZLf-A4tTKVt-RXkCUlaJ00"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1ODM4LCJ1c2VyIjo5MTQyfQ:1u51oI:0HOmvIKwlsX6WUwF0B-Vcr_2pa74CWzigctvv6KcI8w"
        try:
            Check.equal('dolzine_mesecev(1900)', [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31])
            Check.equal('dolzine_mesecev(2000)', [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31])
            Check.equal('dolzine_mesecev(2004)', [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31])
            Check.equal('dolzine_mesecev(2011)', [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31])
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1ODM5LCJ1c2VyIjo5MTQyfQ:1u51oI:krEXvvFv5isuj8h7ZY4dK-_vQ6WoT8VKgaoCP3Y_Qig"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1ODQwLCJ1c2VyIjo5MTQyfQ:1u51oI:-22Vxi-VUM69v4ZYLFGbhEjJir1KMnvzKEbadEAPp90"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1ODQxLCJ1c2VyIjo5MTQyfQ:1u51oI:FHQOD6muPdyQtwd_ZK3JvXQHYYALrELAwlzyBmlm_fY"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1ODQyLCJ1c2VyIjo5MTQyfQ:1u51oI:7ymkJ2xclXtP0s8RZCxDciuH_IHrR3BFugnUNoBYH4A"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1ODQzLCJ1c2VyIjo5MTQyfQ:1u51oI:DE9rUFMtRto4AfgCEBPsls3y3JmnjPJuKFTi89YykhM"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1ODQ0LCJ1c2VyIjo5MTQyfQ:1u51oI:bagGWip_NcMeRW8-uUU1UwiybGAPTeTfxRlO-KbolCM"
        try:
            Check.equal('Datum(31, 12, 1999) == Datum(1, 1, 2000)', False)
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1ODQ1LCJ1c2VyIjo5MTQyfQ:1u51oI:r4u77OhHONay2hEYlSanMbouR97S7q_S-axC4imRcdo"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1ODQ2LCJ1c2VyIjo5MTQyfQ:1u51oI:QyaBV7fpx55RJI1BqVMmyXpfTPjea_kT-VsTTAj6QXE"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1ODQ3LCJ1c2VyIjo5MTQyfQ:1u51oI:zKF9oSBP5btIaVijyAy6U09pSkajrSeAYPM1R-EPTuI"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1ODQ5LCJ1c2VyIjo5MTQyfQ:1u51oI:c74_b5DdevsWfcVO0PToEVQFE-b_zM7zP-BU2N40Ev8"
        try:
