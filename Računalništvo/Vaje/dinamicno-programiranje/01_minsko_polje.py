...
# =====================================================================@005830=
# 1. podnaloga
# Robotka moramo prepeljati čez minirano območje, ki je pravokotne oblike in je razdeljeno
...
def stevilo_poti(n, m, mine):
    poti = [[0 for _ in range(m)] for _ in range(n)]
    poti[n-1][m-1] = 1
    
    for x in range(m - 1):
        if (n - 1, m - x - 1) not in mine:
            poti[n-1][m-x-2] = poti[n-1][m-x-1]
    for y in range(n - 1):
        if (n - y - 1, m - 1) not in mine:
            poti[n-y-2][m-1] = poti[n-y-1][m-1]
    
    for i in range(n-1):
        for j in range(m-1):
            if (n - i - 2, m - j - 2) not in mine:
                poti[n - i - 2][m - j - 2] = poti[n - i - 2][m - j - 1] + poti[n - i - 1][m - j - 2]
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1ODMwLCJ1c2VyIjo5MTQyfQ:1u2UGz:h-FZMdcjnu7FljQY1CxmWE0ke9p5U8LMJiocJSxnidc"
        try:
            test_data = [
                ("""stevilo_poti(5, 4, [])""", 35),
                ("""stevilo_poti(5, 4, [(1, 2), (3, 2)])""", 9),
                ("""stevilo_poti(5, 4, [(2, 2)])""", 17),
                ("""stevilo_poti(5, 4, [(1, 0), (1, 2), (2, 3), (3, 1), (3, 3), (4, 1)])""", 1),
                ("""stevilo_poti(5, 4, [(0, 3), (1, 2), (2, 1), (3, 0)])""", 0),
                ("""stevilo_poti(50, 40, [])""", 14949040860667351485471600),
                ("""stevilo_poti(50, 40, [(2, 3), (2, 10), (2, 30), (5, 2), (7, 19), (40, 30)])""", 6132100941099086511785845),
            ]
            for td in test_data:
                Check.equal(*td)
        except TimeoutError:
