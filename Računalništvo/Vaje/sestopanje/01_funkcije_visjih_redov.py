...
#     3.5
# =============================================================================
linearna = lambda a, b: lambda x: a * x + b
# =====================================================================@005784=
# 2. podnaloga
...
#     1.0
# =============================================================================
kompozitum = lambda f, g: lambda x: f(g(x))
# =====================================================================@005785=
# 3. podnaloga
...
#     2.000000000002
# =============================================================================
#odvod(f, epsilon=10e-5):
#    return lambda x: (f(x + epsilon/2) - f(x - epsilon/2)) / epsilon
odvod = lambda f, eps=10e-5: lambda x: (f(x + eps/2) - f(x - eps/2)) / eps
# =====================================================================@005786=
# 4. podnaloga
...
#     4
# =============================================================================
uporabi_nkrat = lambda f, x, n: x if n == 0 else uporabi_nkrat(f, f(x), n-1)

# Opomba: Če bi nalogo reševal kot normalna oseba, bi uporabil for zanko. Ker zanko uporabljam samo za štetje korakov,
# namesto i uporabimo _, torej: for _ in range(n). To pomeni, da te spremenljivke ne bomo uporabljali.
...
# =============================================================================
uporabi_vse = lambda sez, x: [f(x) for f in sez]

# Opomba: range(len(seznam)) in nato klicanje seznam[i] jezi asistenta. Včasih prav pride,
# ampak tudi v takih primerih lahko stvar pogosto naredimo z enumerate(seznam)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1NzgyLCJ1c2VyIjo5MTQyfQ:1tnHMM:-s3Z9wC-6GZDsR6oYi-ZBbz5n-B-kAjsuxRlxU8DH5c"
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1Nzg0LCJ1c2VyIjo5MTQyfQ:1tnHMM:wLRW_wC4DCw70uy7XMwKH2Td91J0myt_t8QhM7v5p10"
        try:
            import math
            test_data = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1Nzg1LCJ1c2VyIjo5MTQyfQ:1tnHMM:ddUEYlqUBDO91wQ5qLRl3xITjxFg_eA2MFFbm4ng_Gs"
        try:
            import math
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1Nzg2LCJ1c2VyIjo5MTQyfQ:1tnHMM:x-nj7e83NR6G8r6tkfwrsSu8lVMLoRfY3oHqjID_Bmg"
        try:
            test_data = [
                (["collatz = lambda x: x // 2 if x % 2 == 0 else 3*x + 1",
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1Nzg3LCJ1c2VyIjo5MTQyfQ:1tnHMM:zsQ_oG27A0CKzoJPlH2I3fVBSIa3jADco7GyqGfqrb8"
        try:
            test_data = [
                (["f = lambda x: x**2 - 3*x + 4",
                  "g = lambda x: x**x",
