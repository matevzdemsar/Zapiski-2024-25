...
# 
# Sestavite funkcijo `najdi(ime, priimek, i, p)`, ki sprejme seznama `ime`
# in `priimek` z imeni in priimki ter vrne *indeks*, na katerem se pojavlja
# oseba z imenom `i` in priimkom `p`. Funkcija naj išče s postopkom *bisekcije*.
# Če osebe ni, naj vrne `None`. Primer uporabe:
# 
#     >>> ime = ['Ana', 'Mojca', 'Bine', 'Alen', 'Janko', 'Ervin']
#     >>> priimek = ['Ban', 'Ban', 'Bobič', 'Drenik', 'Kovač', 'Lužar']
...
#     1
# 
# Namig: nalogo reši z običajno bisekcijo, le da moraš znati pravilno primerjati
# pare priimek-ime po velikosti.
# =============================================================================
def najdi(ime, priimek, i, p):
    a = [par for par in zip(priimek, ime)]
    x = (p, i)
    def razpolovi(i, j):
        if j < i:
            return None
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1ODE2LCJ1c2VyIjo5MTQyfQ:1tsM1M:2BjPyD5wBEAyFlf02Wv22ILeN73TOjHrPe7HKSBZ5sg"
        try:
            Check.equal("""najdi(['Ana', 'Mojca', 'Bine', 'Alen', 'Janko', 'Ervin'],
                                 ['Ban', 'Ban', 'Bobič', 'Drenik', 'Kovač', 'Lužar'],
                                 'Mojca', 'Ban')""", 1)
            Check.equal("""najdi(['Bolek', 'Lolek'],
                                 ['Nehrebecki', 'Nehrebecki'],
                                 'Bolek', 'Nehrebecki')""", 0)
            Check.equal("""najdi(['Ana', 'Mojca', 'Bine', 'Alen', 'Janko', 'Ervin'],
                                 ['Ban', 'Ban', 'Bobič', 'Drenik', 'Kovač', 'Lužar'],
                                 'Ervin', 'Lužar')""", 5)
